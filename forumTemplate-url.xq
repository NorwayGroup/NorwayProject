...
(: Global variables :)
declare variable $validateUri := "yacht-forum.co.uk";
declare variable $isFirstPostCountedInRepliesPerPage := true();
declare variable $forumUsesPaginationMultiplyer := true();
...
{
    ("http://www.yacht-forum.co.uk/viewtopic.php\?t\d+")
};
...
{
    let $threadNodes := $rootNode//h:a[@class="topiclink"]/ancestor::h:tr[1]
...
{
    let $threadLink := xs:string($threadNode//h:a[@class="topiclink"]/@href)

    let $threadLink := concat("http://www.yacht-forum.co.uk/",$threadLink)
...
{
    let $threadId := ig:getParamValueFromQuery( $threadLink, "t", "" )
...
{
   $threadNode/h:td[3]/h:span[@class="gen"]
};
...
{
    <thread>{$threadLink}&amp;start={$page}</thread>
};

(:11 - Get the year of the thread:)
declare function t:getDateOfLastMessage($threadNode as node()) as xs:string*
{
    let $date := ig:nsj($threadNode/h:td[last()]//text())
    let $date := ig:extract($date, "\w+\s\d{1,2}, \d{4}\s\d{1,2}:\d{2} [ap]m")
    return $date
};
...
{
    ig:getParamValueFromQuery( $documentUri, "f", "" )
};
...
{
    let $a := $rootNode//h:span[@class="pagination"]//h:a[contains(@href,"viewforum.php")]/@href
    let $pages := for $link in $a return xs:integer( ig:getParamValueFromQuery( $link, "start", "0" ) )
    let $lastPage := (max($pages) div t:getThreadsPerPage()) + 1
...
{
    let $currentPage := xs:integer(ig:getParamValueFromQuery( $documentUri, "start", "0" ))
...
{
    let $forumUrl := "http://www.yacht-forum.co.uk/viewforum.php"
    let $forumUrl := ig:setParamValueFromQuery($forumUrl, "f", $forumId)
    let $forumUrl := ig:setParamValueFromQuery($forumUrl, "start", xs:string($page))
