...
(: Global variables :)
declare variable $validateUri := "venturebeat.com";
...
declare variable $useTimeLimitationOnComment := true();
...
{
    if ($documentUri = "http://venturebeat.com/" or matches($documentUri, "^http://venturebeat.com/page/\d+/$"))
    then <GENERATEURL-ONLY/>
    else
         <JAVASCRIPTBLOG/>
};
...
{    
    let $page := if ($documentUri = "http://venturebeat.com/")
                 then 1
                 else xs:integer(ig:extractEx($documentUri, "/page/(\d+)/")) 
                 
    let $pageLink := if ($page <= 50)
                     then concat("http://venturebeat.com/page/", xs:string($page + 1), "/")
                     else ""
...
{
    $blogPost//h:div[@class = "entry-meta-left"]
};
...
{
    $authorNode//h:a[matches(@href, "/author/")]
};

(:5 - Find the blog post author url:)
declare function t:getBlogPostAuthorUrl($authorUrlNode as node()*) as xs:string
{
    let $authorUrl := xs:string($authorUrlNode/@href)
    
    return if(string-length($authorUrl) = 0)
               then
                   ""
               else
                   $authorUrl   
...
{
   let $author := if (string-length($authorUrl)>0)
                  then
                      ig:nsj($authorLinkNode//text())
                  else
                      ig:nsj($authorNode//text())
...
{
    ig:extract(ig:nsj($blogPost//h:*[@class = "the-time"]//text()), "\w+\s\d{1,2},\s\d{4}")
};
...
{
    ig:extractEx($blogPost/@id, "post-(\d+)")
};

(:10 - Fetch the blog post message and refine it:)
declare function t:getBlogPostMessage($blogPost as node()*) as node()*
{
    let $messageNode := $blogPost//h:div[@class = "post-content"]
    let $messageNode := ig:removeTagByName($messageNode, "style")
    let $messageNode := ig:removeTagByNodes($messageNode , $messageNode//h:div[@class = "zemanta-related"]/(following::node() | .))
    let $messageNode := ig:removeTagByAttribute($messageNode,"class","post-boilerplate boilerplate-before")
    let $messageNode := ig:removeTagByAttribute($messageNode,"id","jp-post-flair")
...
{
    $rootNode//h:article[matches(@data-message-id, "\d+")]
};
...
{
    let $parentNode := $comment/parent::h:div[matches(@class, "fyre-comment-replies")]/ancestor::h:article[matches(@data-message-id, "\d+")][1]
    let $parentId := if (exists($parentNode))
                     then ig:extractEx($parentNode/@data-message-id, "(\d+)")
                     else ""
    return $parentId                 
};
...
{
    ($comment/h:div[matches(@class, "fyre-comment-wrapper")]//h:a[@class="fyre-comment-username"])[1]
};
...
{
    let $blog := xs:integer(concat(ig:extractEx($blogInfo/timestamp , "(\d{2})-\d{2}-\d{4}") , ig:extractEx($blogInfo/timestamp , "\d{2}-(\d{2})-\d{4}")))
    let $currentTime := t:generateCommentTime($blogInfo)
    let $currentTime := replace($currentTime , tokenize($currentTime , " ")[1] , " ")
    let $temp := ig:nsj(($comment/h:div[matches(@class, "fyre-comment-wrapper")]//h:time[@class = "fyre-comment-date"])[1])
    let $commentNum := if(string-length($temp))
                       then xs:integer(ig:extract(concat(t:translateMonth(tokenize($temp , " ")[last()] , t:getLanguage()) , ig:extract($temp , "\d+")) , "\d+"))
                       else 0
    let $year := if($commentNum < $blog)
                 then string(xs:integer(ig:extractEx($blogInfo/timestamp , "\d{2}-(\d{4})")) + 1)
                 else ig:extractEx($blogInfo/timestamp , "\d{2}-(\d{4})")
    let $time := ig:nsj($comment/h:div[matches(@class, "fyre-comment-wrapper")]//h:meta[@itemprop = "dateCreated"]/@content)
    let $time := if(string-length($time))
                 then $time
                 else if(matches($temp , "\D+$"))
                      then if(exists(tokenize($temp , " ")[2]))
                           then concat($year , " " , tokenize($temp , " ")[2] , " " , tokenize($temp , " ")[1] , $currentTime)
                           else $temp
                      else $temp
    return $time   
};
...
{
    ig:extractEx($comment/@data-message-id, "(\d+)")
};
...
{
    concat($documentUri, "#comment-", $commentId)
};

(:12 Find and refine the comment message:)
declare function t:getCommentMessage($comment as node()*) as node()*
{
   ($comment/h:div[matches(@class, "fyre-comment-wrapper")]//h:div[@class = "fyre-comment"])[1]  
};
(: COMMENTS - END :)
